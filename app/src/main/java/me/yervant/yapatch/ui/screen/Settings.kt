...
import me.yervant.yapatch.ui.component.SwitchItem
import me.yervant.yapatch.ui.component.rememberConfirmDialog
import me.yervant.yapatch.ui.component.rememberLoadingDialog
import me.yervant.yapatch.ui.theme.refreshTheme
import me.yervant.yapatch.util.YAPatchKeyHelper
import me.yervant.yapatch.util.getBugreportFile
import me.yervant.yapatch.util.isForceUsingOverlayFS
import me.yervant.yapatch.util.isGlobalNamespaceEnabled
import me.yervant.yapatch.util.isLiteModeEnabled
import me.yervant.yapatch.util.outputStream
import me.yervant.yapatch.util.overlayFsAvailable
import me.yervant.yapatch.util.rootShellForResult
import me.yervant.yapatch.util.setForceUsingOverlayFS
import me.yervant.yapatch.util.setGlobalNamespaceEnabled
import me.yervant.yapatch.util.setLiteMode
import me.yervant.yapatch.util.ui.APDialogBlurBehindUtils
import me.yervant.yapatch.util.ui.LocalSnackbarHost
import me.yervant.yapatch.util.ui.NavigationBarsSpacer
...
fun SettingScreen() {
    ...
    var bSkipStoreSuperKey by rememberSaveable {
        mutableStateOf(YAPatchKeyHelper.shouldSkipStoreSuperKey())
    }
    val isOverlayFSAvailable by rememberSaveable {
        mutableStateOf(overlayFsAvailable())
    }
    if (kPatchReady && YAPatchReady) {
    ...
    Scaffold(
        ...
        val clearKeyDialog = rememberConfirmDialog(
            onConfirm = {
                YAPatchKeyHelper.clearConfigKey()
        ...
        val showDarkThemeChooseDialog = remember { mutableStateOf(false) }
        if (showDarkThemeChooseDialog.value) {
            DarkThemeChooseDialog(showDarkThemeChooseDialog)
        }
        ...
        Column(
            ...
            SwitchItem(
                ...
                onCheckedChange = {
                    ...
                    YAPatchKeyHelper.setShouldSkipStoreSuperKey(bSkipStoreSuperKey)
                })
            ...
            // Force OverlayFS
            if (kPatchReady && YAPatchReady && isOverlayFSAvailable) {
                SwitchItem(
            ...
            if (!nightFollowSystem) {
                ListItem(
                    headlineContent = { Text(text = stringResource(id = R.string.settings_dark_theme)) },
                    modifier = Modifier.clickable {
                        showDarkThemeChooseDialog.value = true
                    },
                    supportingContent = {
                        val darkThemeMode = prefs.getString("dark_theme_mode", "off")
                        Text(
                            text = stringResource(darkThemeModeToString(darkThemeMode.toString())),
                                    ...
                                    modifier = Modifier
                                        ...
                                        .clickable {
                                            scope.launch {
                                                ...
                                                exportBugreportLauncher.launch("YAPatch_bugreport_${current}.tar.gz")
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun DarkThemeChooseDialog(showDialog: MutableState<Boolean>) {
    val prefs = APApplication.sharedPreferences
    ...
    BasicAlertDialog(
        ...
        Surface(
            ...
            LazyColumn {
                items(darkThemeModes()) {
                    ListItem(
                        headlineContent = { Text(text = stringResource(it.nameId)) },
                        modifier = Modifier.clickable {
                            showDialog.value = false
                            prefs.edit { putString("dark_theme_mode", it.name) }
                            refreshTheme.value = true
                        })
                }
...
private data class DarkThemeMode(
    val name: String, @param:StringRes val nameId: Int
)

private fun darkThemeModes(): List<DarkThemeMode> {
    return listOf(
        DarkThemeMode("off", R.string.off),
        DarkThemeMode("on", R.string.on),
        DarkThemeMode("amoled", R.string.settings_theme_amoled),
    )
}

@Composable
private fun darkThemeModeToString(darkThemeMode: String): Int {
    return darkThemeModes().find { it.name == darkThemeMode }?.nameId ?: R.string.off
}
