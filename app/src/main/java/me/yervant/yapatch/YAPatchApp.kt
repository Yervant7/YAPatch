...
class APApplication : Application(), Thread.UncaughtExceptionHandler {
    ...
    companion object {
        ...
        const val YAPatch_FOLDER = "/data/adb/ap/"
        private const val YAPatch_BIN_FOLDER = YAPatch_FOLDER + "bin/"
        private const val YAPatch_LOG_FOLDER = YAPatch_FOLDER + "log/"
        ...
        const val KPMS_DIR = YAPatch_FOLDER + "kpms/"

        @Deprecated("Use 'apd -V'")
        const val YAPatch_VERSION_PATH = YAPatch_FOLDER + "version"
        private const val MAGISKPOLICY_BIN_PATH = YAPatch_BIN_FOLDER + "magiskpolicy"
        private const val BUSYBOX_BIN_PATH = YAPatch_BIN_FOLDER + "busybox"
        private const val RESETPROP_BIN_PATH = YAPatch_BIN_FOLDER + "resetprop"
        private const val MAGISKBOOT_BIN_PATH = YAPatch_BIN_FOLDER + "magiskboot"
        ...
        fun uninstallYAPatch() {
            ...
            val cmds = arrayOf(
                "rm -f $APD_PATH",
                "rm -rf $YAPatch_BIN_FOLDER",
                "rm -rf $YAPatch_LOG_FOLDER",
                "rm -rf $YAPatch_VERSION_PATH",
            )

            val shell = getRootShell()
            shell.newJob().add(*cmds).to(logCallback, logCallback).exec()

            Log.d(TAG, "YAPatch uninstalled...")
        ...
        fun installYAPatch() {
            ...
            val cmds = arrayOf(
                "mkdir -p $YAPatch_BIN_FOLDER",
                "mkdir -p $YAPatch_LOG_FOLDER",
        ...
        var superKey: String = ""
            set(value) {
                ...
                YAPatchKeyHelper.writeSPSuperKey(value)
                ...
                .addInterceptor { block ->
                    block.proceed(
                        block.request().newBuilder()
                            .header("User-Agent", "YAPatch/${BuildConfig.VERSION_CODE}")
