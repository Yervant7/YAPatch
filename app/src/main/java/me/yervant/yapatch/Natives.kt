...
object Natives {
    ...
    data class SuperCallConfig(
        var hideFiles: List<String> = emptyList(),
        var hideVma: List<String> = emptyList(),
        var spoofStatFiles: List<String> = emptyList(),
        var spoofUname: Map<Int, String> = emptyMap()
    ) : Parcelable
    ...
    @FastNative
    private external fun nativeAndroidSpoofUnameRemove(superKey: String, field: Int): Int
    fun androidSpoofUnameRemove(field: Int): Int {
        return nativeAndroidSpoofUnameRemove(APApplication.superKey, field)
    }
}
